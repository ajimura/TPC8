...
static const char* dumgenb_spec[] =
{
    "implementation_id", "DumGenB",
    "type_name",         "DumGenB",
    "description",       "DumGenB component",
    "version",           "1.0",
...
DumGenB::DumGenB(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("dumgenb_out", m_out_data),
      m_recv_byte_size(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("dumgenb_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("DUMGENB");
}
...
RTC::ReturnCode_t DumGenB::onInitialize()
{
    if (m_debug) {
        std::cerr << "DumGenB::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t DumGenB::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int DumGenB::daq_dummy()
{
    return 0;
}

int DumGenB::daq_configure()
{
    std::cerr << "*** DumGenB::configure" << std::endl;
...
int DumGenB::parse_params(::NVList* list)
{
  int i;
...
int DumGenB::daq_unconfigure()
{
    std::cerr << "*** DumGenB::unconfigure" << std::endl;
...
int DumGenB::daq_start()
{
    std::cerr << "*** DumGenB::start" << std::endl;
    m_out_status = BUF_SUCCESS;
    eventnum = 0;
...
int DumGenB::daq_stop()
{
    std::cerr << "*** DumGenB::stop" << std::endl;
    return 0;
}

int DumGenB::daq_pause()
{
    std::cerr << "*** DumGenB::pause" << std::endl;
    return 0;
}

int DumGenB::daq_resume()
{
    std::cerr << "*** DumGenB::resume" << std::endl;
    return 0;
}

int DumGenB::read_data_from_detectors()
{
...
extern "C"
{
    ...
    {
        RTC::Properties profile(dumgenb_spec);
...
void DumGenB::toLower(std::basic_string<char>& s)
{
